...
export function RollingMetricValue({ value }: RollingMetricValueProps) {
  ...
  useGSAP(
    () => {
      ...
      const tracks = gsap.utils.toArray<HTMLSpanElement>(
        rootRef.current.querySelectorAll('[data-digit-track]'),
      );

      if (digitTokens.length === 0 || tracks.length !== digitTokens.length) {
        return;
      }
      ...
      const targetSteps = digitTokens.map(
        (token, index) => (2 + index) * 10 + token.digit,
      );
